...
class ActivityDAOTest {
    ...
    inner class ReadActivities {
        ...
        @Test
        fun `get activity by user id that exists, results in a correct activities(s) returned`() {
            transaction {
                //Arrange - create and populate tables with three users and three activities
                populateUserTable()
                val activityDAO = populateActivityTable()
                ...
                assertEquals(activity2, activityDAO.findByUserId(2)[0])
                assertEquals(activity3, activityDAO.findByUserId(2)[1])
        ...
        fun `updating existing activity in table results in successful update`() {
            transaction {
                ...
                val activity3updated = Activity(
                    ...
                    calories = 220, started = DateTime.now(), userId = 2, fitnessId = 1
                )
    ...
    inner class DeleteActivities {
        ...
        fun `deleting activities when none exist for user id results in no deletion`() {
            transaction {
                ...
                //Act & Assert
                assertEquals(3, activityDAO.getAll().size)
                activityDAO.deleteAllByUserId(3)
                assertEquals(3, activityDAO.getAll().size)
            }
        }
        ...
        fun `deleting activities when 1 or more exist for user id results in deletion`() {
            transaction {
                ...
                //Act & Assert
                assertEquals(3, activityDAO.getAll().size)
                activityDAO.deleteAllByUserId(1)
                assertEquals(2, activityDAO.getAll().size)
            }
        }
    }
